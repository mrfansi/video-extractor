...
def create_sample_video(duration=10, size="1280x720"):
    """
    ...
    """
    print(f"Creating sample video with resolution {size} and duration {duration}s...")
    
    # Create a more complex test video with FFmpeg
    cmd = [
        'ffmpeg',
        ...
        '-c:v', 'libx264',
        '-b:v', '5M',  # Higher bitrate for more realistic test
        '-pix_fmt', 'yuv420p',
        TEST_VIDEO_PATH
    ]
    ...
    try:
        ...
        print(f"Sample video created at {TEST_VIDEO_PATH}")
...
def main():
    ...
    # Create sample video
    sample_video = create_sample_video(duration=10, size="1280x720")
    if not sample_video:
        print("Failed to create sample video. Exiting.")
        return
